...
void recDFS(SquareMaze& maze, vector<vector<uint8_t> >* visited, int x, int y,
  ...
  while (!s.empty()) {
    ...
    if (x < width - 1 && maze.canTravel(x, y, 0)) s.push(make_pair(x + 1, y));
    if (y < height - 1 && maze.canTravel(x, y, 2)) s.push(make_pair(x, y + 1));
    if (x > 0 && maze.canTravel(x, y, 1)) s.push(make_pair(x - 1, y));
    if (y > 0 && maze.canTravel(x, y, 3)) s.push(make_pair(x, y - 1));
  }
}

pair<int, int> assert_maze_helper(SquareMaze& maze, int width, int height) {
  vector<vector<uint8_t> > visited;
  for (int i = 0; i < width; i++) visited.push_back(vector<uint8_t>(height));
...
void assert_maze_acyclic(SquareMaze& maze, int width, int height) {
  ...
  if (calls + components != width * height * 2) FAIL("Maze has a cycle");
}
...
void assert_maze_connected(SquareMaze& maze, int width, int height) {
  ...
  int components = checks.first;
  int calls = checks.second;
  if (components != 1) FAIL("Maze is not connected");
}
...
TEST_CASE("testSolveMazeValidPath", "[weight=10][part2]") {
  ...
  if (solution.empty()) FAIL("No solution was generated");
...
TEST_CASE("testSolutionBottomRow", "[weight=10][part2]") {
  ...
  if (y != soln.getDestinationY()) FAIL("Didn't end up at the bottom row");
}
